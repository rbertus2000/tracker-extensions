...
//#endregion
//#region src/MangaUpdates/Implementations/Shared/parser/manga.ts
	const ADULT_GENRES = [
		"Adult",
		"Hentai",
		"Smut"
	...
	const unsafeGenres = [...ADULT_GENRES, ...MATURE_GENRES];
	function parseStatus(status) {
		const statusMatches = /\(([a-zA-Z]+)\)/g.exec(status)?.slice(1).map((match) => match.toLowerCase()) || [];
		if (statusMatches.some((match) => match.includes("ongoing"))) return "ONGOING";
		if (statusMatches.some((match) => match.includes("hiatus"))) return "HIATUS";
		if (statusMatches.some((match) => match.includes("incomplete") || match.includes("discontinued"))) return "ABANDONED";
		if (statusMatches.some((match) => match.includes("complete"))) return "COMPLETED";
		return "UNKNOWN";
	}
	function getContentRating(manga) {
		const genres = (manga.genres ?? []).map((g) => g.genre).filter((g) => g != null);
		if (genres.some((g) => ADULT_GENRES.includes(g))) return ContentRating.ADULT;
		if (genres.some((g) => MATURE_GENRES.includes(g))) return ContentRating.MATURE;
		return ContentRating.EVERYONE;
	}
	function parseMangaInfo(series) {
		const titles = [series.title, ...(series.associated || []).map((associated) => associated?.title)].filter((title) => !!title);
		return {
...
//#endregion
//#region src/MangaUpdates/Implementations/Shared/parser/session.ts
	const logPrefix = "[mu-session]";
	const STATE_MU_SESSION = "mu_sessiontoken";
	function getSessionToken() {
		const sessionToken = Application.getSecureState(STATE_MU_SESSION);
	...
	function getSessionInfo() {
		try {
			...
			const payloadJson = Application.base64Decode(payloadBase64);
			const payload = JSON.parse(payloadJson);
			const loginTime = /* @__PURE__ */ new Date(payload.time_created * 1e3);
			return {
	...
	function assertMustBeAuthenticated() {
		if (getSessionInfo() == null) throw new Error("You are not authenticated, please log in through the MangaUpdates settings");
	}

//#endregion
//#region src/MangaUpdates/Services/Requests.ts
	function loggableRequest(request) {
		let censoredRequest = request;
		if (censoredRequest.body?.password) censoredRequest = {
			...request,
			body: {
				...request.body,
				password: "***"
		...
		if (censoredRequest.params?.authHash) censoredRequest = {
			...censoredRequest,
			params: {
	...
	function loggableResponse(response) {
		...
		if (censoredResponse?.context?.session_token) censoredResponse = {
			...censoredResponse,
			context: {
	...
	async function makeRequest(endpoint, verb, request, { allowAnonymous = false, failOnError = true } = {}) {
		...
		if (baseRequest.body) headers["content-type"] = "application/json";
		const sessionToken = getSessionToken();
		if (!allowAnonymous) {
			if (failOnError) assertMustBeAuthenticated();
		...
		const params = {
			...
			method: verb,
			headers,
			body: baseRequest.body ? JSON.stringify(baseRequest.body) : void 0
		};
		...
		try {
			...
			console.log(e);
			if (failOnError) throw new Error("Request error!");
			return;
		}
		...
		let responseBody;
		try {
			responseBody = JSON.parse(Application.arrayBufferToUTF8String(bodyBuffer));
		} catch (e) {
			console.log(`${logPrefix} failed ${meta}: BODY PARSING FAILED`);
			console.log(e);
			if (failOnError) throw new Error("Unable to parse response body!");
			return;
		}
...
//#endregion
//#region src/MangaUpdates/Implementations/DiscoverSection/main.ts
	var DiscoverSectionImplementation = class {
		...
		async getDiscoverSectionItems(section, metadata) {
			...
			try {
				...
				const page = await makeRequest("/v1/series/search", "POST", { body }, {
					allowAnonymous: true,
					failOnError: false
				});
				const results = (page?.results ?? []).map((r) => r.record);
				const items = results.filter((r) => r != null).map((r) => ({
					...parseMangaInfo(r),
				...
				console.log(`${logPrefix} complete: ${section.id}, page=${body.page} nextPage=${nextPage} results=${results.length} resultsAvailable=${resultsAvailable}`);
				return {
					items,
					metadata: nextPage
				};
...
//#endregion
//#region src/MangaUpdates/Implementations/helper.ts
...
//#endregion
//#region src/MangaUpdates/Implementations/Manga/main.ts
...
//#endregion
//#region src/MangaUpdates/Implementations/MangaProgress/form.ts
	var DeletionForm = class extends Form {
		constructor(mangaId, onFormWillClose) {
			super();
			this.mangaId = mangaId;
			this.onFormWillClose = onFormWillClose;
		...
		getSections() {
			if (this.result != null) {
				...
				return [Section("deleted", [LabelRow("deleted", {
					title: success ? "Deleted" : this.result.status.toUpperCase(),
					subtitle: success ? "The title has been succesfully deleted from your media list" : this.result.reason
				})])];
			}
			return [Section({
				id: "delete",
				...
				title: "Delete",
				onSelect: Application.Selector(this, "onDelete")
			})])];
		}
		async onDelete() {
			...
			try {
				...
				const result = await makeRequest("/v1/lists/series/delete", "POST", { body: [parseInt(this.mangaId)] }, { failOnError: false });
				if (result == null) throw new Error("Failed to delete.");
				this.result = result;
	...
	var MangaProgressForm = class extends Form {
		constructor(mangaId) {
			super();
			this.mangaId = mangaId;
			_defineProperty(this, "loading", true);
			_defineProperty(this, "loadingMessage", null);
			_defineProperty(this, "initialState", null);
		...
		getSections() {
			if (this.loading) return [Section("loading", [LabelRow("loading", {
				title: this.loadingMessage ?? "Loading...",
				subtitle: this.loadingMessage ? "This is caused by MangaUpdates API rate limits. We'll be done as fast as we can." : ""
			})])];
			if (this.error != null || this.state == null) return [Section("error", [LabelRow("error", {
				title: "Error",
			...
			return [
				...
				Section({
					...
					SelectRow("list-id", {
						...
						minItemCount: 0,
						maxItemCount: 1,
						onValueChange: Application.Selector(this, "setList")
					}),
					StepperRow("chapter", {
						title: "Chapters",
						...
						stepValue: 1,
						loopOver: false,
						onValueChange: Application.Selector(this, "setChapter")
					}),
					StepperRow("volume", {
						title: "Volumes",
						...
						stepValue: 1,
						loopOver: false,
						onValueChange: Application.Selector(this, "setVolume")
					})
				]),
				Section({
					id: "score",
					...
					stepValue: .1,
					loopOver: false,
					onValueChange: Application.Selector(this, "setScore")
				})]),
				Section({
					id: "delete",
		...
		async loadData(overrideExistingState = false) {
			...
			try {
				...
				const [mangaInfo, mangaLists, progressInfo, ratingInfo] = await Promise.all([
					...
					makeRequest("/v1/lists/series/{seriesId}", "GET", {
						params: { seriesId: this.mangaId },
						query: {}
					}, { failOnError: false }),
					makeRequest("/v1/series/{id}/rating", "GET", { params: { id: this.mangaId } }, { failOnError: false })
				]);
				...
				this.initialState = {
					...
					listId: progressInfo?.list_id != null ? String(progressInfo.list_id) : void 0,
					listOptions: mangaLists.filter((l) => l.list_id != null && l.title != null).map((l) => ({
						id: String(l.list_id),
						title: l.title
					})),
				...
				if (this.state == null || overrideExistingState) this.state = { ...this.initialState };
		...
		async saveChanges() {
			...
			try {
				...
				console.log(`${logPrefix} start: ${this.mangaId} ${JSON.stringify({
					initialState: prev,
					submittedState: next
				})}`);
				const actions = [];
				const progressChanged = prev.chapter !== next.chapter || prev.volume !== next.volume || prev.listId !== next.listId;
				const list = { body: [{
					series: { id },
				...
				if (progressChanged) if (next.listId == null) actions.push({
					message: "Deleting from list",
					action: async () => makeRequest("/v1/lists/series/delete", "POST", { body: [id] })
				});
				else if (prev.listId == null) actions.push({
				...
				if (prev.score !== next.score) if (next.score > 0) actions.push({
					message: "Adding rating",
					action: () => makeRequest("/v1/series/{id}/rating", "PUT", {
						params: { id },
					...
					action: () => makeRequest("/v1/series/{id}/rating", "DELETE", { params: { id } })
				});
				...
				for (const [i, { message, action }] of actions.entries()) {
					...
					await action();
					if (i < actions.length - 1) await Application.sleep(5);
				}
...
//#endregion
//#region src/MangaUpdates/Implementations/MangaProgress/main.ts
	...
	var MangaProgressImplementation = class {
		...
		async getMangaProgress(sourceManga) {
			...
			const progressInfo = await makeRequest("/v1/lists/series/{seriesId}", "GET", {
			...
			console.log(`${logPrefix} complete: ${JSON.stringify({
		...
		async processChapterReadActionQueue(actions) {
			...
			try {
				...
				const mangaActions = actions.reduce((acc, action) => {
					if (acc[action.sourceManga.mangaId] == null) acc[action.sourceManga.mangaId] = [];
					acc[action.sourceManga.mangaId].push(action);
					return acc;
				}, {});
				const operations = await Promise.all(Object.entries(mangaActions).map(([mangaId, actions]) => this.parseMangaActions(mangaId, actions)));
				const updates = [];
				const additions = [];
				for (const operation of operations) switch (operation.type) {
					...
					case "noop":
						...
						break;
					default: failedItems.push(...operation.ids);
				}
				if (updates.length > 0) try {
					...
					await makeRequest("/v1/lists/series/update", "POST", { body });
				...
				if (updates.length > 0 && additions.length > 0) await Application.sleep(5);
				if (additions.length > 0) try {
		...
		async parseMangaActions(mangaId, actions) {
			try {
				const seriesId = parseInt(mangaId);
				const listInfo = await makeRequest("/v1/lists/series/{seriesId}", "GET", {
					params: { seriesId },
					query: {}
				}, { failOnError: false });
				const listChapter = listInfo?.status?.chapter ?? 0;
				const listVolume = listInfo?.status?.volume ?? 0;
				const actionChapter = Math.max(...actions.map((a) => Math.floor(a.chapterNum || 1)), listChapter);
				const actionVolume = Math.max(...actions.map((a) => Math.floor(a.chapterVolume || 1)), listVolume);
				let type;
				if (listInfo == null) type = "addition";
				else if (actionChapter > listChapter || actionVolume > listVolume) type = "update";
				else type = "noop";
				return {
...
//#endregion
//#region src/MangaUpdates/Implementations/SearchResults/form.ts
	const TYPE_OPTIONS = [
		"Artbook",
		"Doujinshi",
		"Drama CD",
	...
	const STATUS_OPTIONS = [
		...
		{
			id: "scanlated",
			title: "Only show completely scanlated manga"
		},
		{
			id: "completed",
			title: "Only show completed (including oneshots) manga"
		},
		{
			id: "oneshots",
			title: "Only show one shots"
		},
		{
			id: "no_oneshots",
			title: "Exclude one shots"
		},
		{
			id: "some_releases",
			title: "Only show manga with at least one release"
		},
		{
			id: "no_releases",
			title: "Only show manga with no releases"
		}
	];
	...
	var MangaUpdatesAdvancedSearchForm = class extends AdvancedSearchForm {
		constructor(searchQuery) {
			super();
			_defineProperty(this, "demographicOptions", []);
			...
			_defineProperty(this, "demographics", void 0);
			_defineProperty(this, "genres", void 0);
			_defineProperty(this, "categories", void 0);
			_defineProperty(this, "licensed", void 0);
			_defineProperty(this, "types", void 0);
			_defineProperty(this, "status", void 0);
			_defineProperty(this, "list", void 0);
			this.loadOptions().then(({ genres, lists }) => {
				this.demographicOptions = genres.filter((g) => g.demographic && g.genre).map((g) => ({
					id: g.genre.replaceAll(" ", "_"),
					title: g.genre
				}));
				this.genreOptions = genres.filter((g) => !g.demographic && g.genre).map((g) => ({
					id: g.genre.replaceAll(" ", "_"),
					title: g.genre
				}));
				this.listOptions = lists.filter((l) => l.list_id != null && l.title != null).map((l) => ({
					id: String(l.list_id),
					title: l.title
				}));
		...
		async loadOptions() {
			if (Number(Application.getState(STATE_QUERY_DATE) ?? 0) + CACHE_TTL_SECONDS > (/* @__PURE__ */ new Date()).valueOf() / 1e3) return {
				genres: JSON.parse(Application.getState(STATE_GENRES)),
				lists: JSON.parse(Application.getState(STATE_LISTS))
			};
			const [genres, lists] = await Promise.all([makeRequest("/v1/genres", "GET", {}, { allowAnonymous: true }), makeRequest("/v1/lists", "GET", {}, { failOnError: false })]);
			Application.setState(JSON.stringify(genres), STATE_GENRES);
			Application.setState(JSON.stringify(lists), STATE_LISTS);
			Application.setState(String((/* @__PURE__ */ new Date()).valueOf() / 1e3), STATE_QUERY_DATE);
			return {
				genres,
				lists: lists ?? []
			};
		...
		getSections() {
			...
			return [Section({
				...
				TriStateSelectRow("demographics", {
					...
					onValueChange: Application.Selector(this, "handleDemographicsChange")
				}),
				...
				InputRow("categories", {
					...
					onValueChange: Application.Selector(this, "handleCategoriesChange")
				}),
				TriStateSelectRow("licensed", {
					...
					allowEmptySelection: true,
					maximum: 1,
					onValueChange: Application.Selector(this, "handleLicensedChange")
				}),
				...
				SelectRow("list", {
					...
					minItemCount: 1,
					maxItemCount: 1,
					onValueChange: Application.Selector(this, "handleListChange")
				})
			])];
		}
		async handleDemographicsChange(value) {
			this.demographics = value;
		}
		async handleGenresChange(value) {
			this.genres = value;
		}
		async handleCategoriesChange(value) {
			this.categories = value;
		}
		async handleLicensedChange(value) {
...
//#endregion
//#region src/MangaUpdates/Implementations/SearchResults/parser.ts
	const SearchOrderBy = {
		none: "none",
		title: "title",
		year: "year",
	...
	function toSearchOrder(sortingOption) {
		if (sortingOption?.id === SearchOrderBy.none) return;
		return sortingOption != null && sortingOption.id in SearchOrderBy ? sortingOption.id : void 0;
	}
	function parseSearchResults(results) {
		return results.map((result) => {
			...
			const contentRating = result.record != null ? getContentRating(result.record) : void 0;
			if (!mangaId || !title) {
...
//#endregion
//#region src/MangaUpdates/Implementations/SearchResults/main.ts
	var SearchResultsImplementation = class extends MangaImplementation {
		...
		async getSortingOptions(query) {
			return [
				{
					id: SearchOrderBy.none,
					label: "Default"
				},
				{
					id: SearchOrderBy.year,
					label: "Year"
				},
				{
					id: SearchOrderBy.title,
					label: "Title"
				},
				{
					id: SearchOrderBy.rating,
					label: "Rating"
				}
			];
		}
		async getSearchResults(query, metadata, sortingOption) {
			...
			try {
				const body = {
					page: metadata ?? 1,
					perpage: 25,
					search: query.title || void 0,
					orderby: toSearchOrder(sortingOption)
				};
				if (body.page < 1) return {
					items: [],
					metadata: { nextPage: -1 }
				};
				...
				if (meta.categories) {
					const categories = meta.categories.split(",").map((c) => c.trim()).filter((c) => c.length > 0);
				...
				if (meta.types) {
					const includedTypes = [];
					for (const [id, state] of Object.entries(meta.types)) if (state === "included") includedTypes.push(id.replaceAll("_", " "));
					if (includedTypes.length > 0) body.filter_types = includedTypes;
				}
				if (meta.status) body.filters = [meta.status];
				if (meta.list) body.list = meta.list;
				console.log(`${logPrefix} searching: ${JSON.stringify(body)}`);
				const response = await makeRequest("/v1/series/search", "POST", { body }, { allowAnonymous: true });
				const results = parseSearchResults(response.results || []);
				const nextPage = body.page * body.perpage < (response.total_hits ?? 0) ? body.page + 1 : -1;
				console.log(`${logPrefix} got results: ${JSON.stringify({
					results,
					nextPage
				})}`);
				const pagedResults = {
...
//#endregion
//#region src/MangaUpdates/Implementations/SettingsForm/form.ts
	var LoginForm = class extends Form {
		constructor(..._args) {
			super(..._args);
		...
		getSections() {
			return [Section({
				...
				onValueChange: Application.Selector(this, "onUsernameChange")
			}), InputRow("password-input", {
				title: "Password",
				value: this.loginForm.password,
				onValueChange: Application.Selector(this, "onPasswordChange")
			})])];
		}
		async onUsernameChange(newUsername) {
			this.loginForm.username = newUsername;
		}
		async onPasswordChange(newPassword) {
		...
		async formDidSubmit() {
			...
			if (!this.loginForm.username || !this.loginForm.password) {
				console.error(`${logPrefix} login called with invalid credentials: ${JSON.stringify(this.loginForm)}`);
				throw new Error("Must provide a username and password!");
			}
			try {
				const result = await makeRequest("/v1/account/login", "PUT", { body: this.loginForm }, { allowAnonymous: true });
	...
	var SettingsForm = class extends Form {
		getSections() {
			...
			if (info == null) return this.unauthenticatedView();
			return this.authenticatedView(info);
		}
		unauthenticatedView() {
			return [Section({ id: "login-section" }, [NavigationRow("login", {
				title: "Log in",
				form: new LoginForm()
			})])];
		}
		authenticatedView(info) {
			return [Section({
				id: "profile-section",
				header: "Profile"
			}, [
			...
			]), Section({ id: "session-section" }, [ButtonRow("logout-button", {
				title: "Log out",
...
//#endregion
//#region src/MangaUpdates/Implementations/SettingsForm/main.ts
...
//#endregion
//#region src/MangaUpdates/Services/Interceptor.ts
	var MangaUpdatesInterceptor = class extends PaperbackInterceptor {
		async interceptRequest(request) {
			return request;
		}
		async interceptResponse(request, response, data) {
...
//#endregion
//#region src/MangaUpdates/main.ts
	var MangaUpdatesExtension = class {
		constructor() {
			...
			_defineProperty(this, "mainInterceptor", new MangaUpdatesInterceptor("main"));
		}
		async initialise() {
	...
	const MangaUpdates = new MangaUpdatesExtension();

//#endregion
//#region src/MangaUpdates/pbconfig.ts
	var pbconfig_default = {
		name: "MangaUpdates",
		description: "Extension that integrates with mangaupdates.com for tracking and collection management.",
...
//#endregion
//#region src/tests/MangaUpdates.ts
	async function runTests(logger) {
		const suite = new TestSuite("MangaUpdates tests", logger);
		registerDefaultTests(suite, MangaUpdates, pbconfig_default);
		await suite.run();
	}
...
//# sourceMappingURL=MangaUpdates.js.map